# E-commerce Product Image Workflow

Welcome to the project documentation.

This project helps teams convert supplier images into publish-ready e-commerce assets while enforcing product consistency, visual differentiation, and auditable QA.

## Start here

- [Quick Start](en/quickstart.md)
- [End-to-End Workflow](en/workflow.md)
- [Quality Gate](en/quality-gate.md)
- [Risk & Compliance](en/compliance.md)
- [MVP CLI](en/quickstart.md#mvp-cli-python)

## Chinese docs

- [中文导航](zh-CN/README.md)
- [原始参考文档目录](zh-CN/reference/README.md)
